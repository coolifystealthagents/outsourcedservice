...
<section className="hero"><div className="container hero-grid"><div className="copy"><p className="eyebrow">Managed staffing plan</p><h1>Hire managed {roleLabel} without screening alone.</h1><p className="lead">Get a role plan built around the work, schedule, tools, and review rules your team already uses.</p><div className="actions"><a className="btn primary" href="/contact">Request staffing plan</a><a className="btn secondary" href="#tasks">Get task ideas</a></div><p className="risk">No public rate card. Share the role first, then get a practical scope.</p></div><div className="match-card"><div className="portrait-wrap"><img src={heroImage} alt={site.alt||`${site.brand||roleLabel} managed staffing visual`}/><span className="badge">Role match</span></div><div className="task-note note-a"><b>Daily handoff</b><span>clear owner brief</span></div><div className="task-note note-b"><b>Quality checks</b><span>review sample work</span></div><div className="task-note note-c"><b>Controlled launch</b><span>scope → shadow → review</span></div></div></div><div className="container proof-bar"><span>Define the work before choosing a hire</span>{stats.map((s:any,i:number)=><b key={i}>{s.value}</b>)}</div></section>
<section className="container section" id="tasks"><div className="split-head"><div><p className="eyebrow">Task ideas</p><h2>Start with work that repeats every week.</h2></div><p>Pick work with a clear example, a named reviewer, and a rule for anything the staff member should not decide alone.</p></div><div className="task-grid">{services.map((s:any,i:number)=><a key={i} href={`/services/${slug(s)}`}><span>{String(i+1).padStart(2,'0')}</span><h3>{title(s)}</h3><p>{text(s)}</p><b>See handoff →</b></a>)}</div></section>

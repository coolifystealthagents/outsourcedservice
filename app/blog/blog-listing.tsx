...
import { blogPosts, generatedBlogPosts, batchBlogPosts } from '../data';

const PAGE_SIZE = 20;

export function BlogListing({ page = 1 }: { page?: number }) {
  const posts = [
    ...batchBlogPosts.map((item) => ({ slug: item.slug, title: item.title, excerpt: item.excerpt, minutes: 10 })),

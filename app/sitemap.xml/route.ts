import aug19Meta from '../aug19-meta.json';import { fleetServices, researchPosts } from '../fleet-content';
...
export async function GET() {
  ...
  const urls = [...pages, ...fleetServices.map((s) => `/services/${s.slug}`), ...allBlog.map((p) => `/blog/${p.slug}`), ...blogPages, ...researchPosts.map((p) => `/research/${p.slug}`), ...aug19Paths];

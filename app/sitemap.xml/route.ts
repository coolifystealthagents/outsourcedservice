import aug19Meta from '../aug19-meta.json';import aug20Meta from '../aug20-meta.json';import { fleetServices, researchPosts } from '../fleet-content';
...
export async function GET() {
  ...
  const allBlog = [...aug20Blog, ...august17BlogPosts, ...august14BlogPosts, ...august13BlogPosts, ...blogPosts, ...generatedBlogPosts.map((item) => ({ slug: item[0] })), ...batchBlogPosts.map((item) => ({ slug: item.slug }))];
  ...
  const urls = [...pages, ...fleetServices.map((s) => `/services/${s.slug}`), ...allBlog.map((p) => `/blog/${p.slug}`), ...blogPages, ...aug20Research.map((p) => `/research/${p.slug}`), ...researchPosts.map((p) => `/research/${p.slug}`), ...aug19Paths];

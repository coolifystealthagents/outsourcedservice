...
MARKER = 'ph-appointment-scheduling-20260728'
...
body = re.search(r'<div class="article-body accounting-guide scheduling-control-guide">(.*)</div>', article, re.S)
if not body:
    raise SystemExit('AUDIT_SELECTOR_MISMATCH: scheduling article body')
paragraphs = []
for paragraph in re.finditer(r'<p(?:\s[^>]*)?>(.*?)</p>', body.group(1), re.I | re.S):
    ...
    paragraphs.append({'text': value[:100], 'sentences': len(parts)})
...
checks = {
    ...
    'h1_exact': h1_text == TITLE,
    'h1_keyword_prefix': h1_text.lower().startswith('philippines appointment scheduling'),
    ...
    'unique_marker': f'data-article-marker="{MARKER}"' in article,
    ...
    'chart_present': 'class="evidence-chart"' in article and 'scheduling-chart-title' in article,
    'graphic_present': 'class="review-graphic"' in article and 'scheduling-graphic-title' in article,

...
MARKER = 'ph-order-control-20260728'
...
body = re.search(r'<div class="article-body accounting-guide order-control-guide">(.*)</div>', article, re.S)
if not body:
    raise SystemExit('AUDIT_SELECTOR_MISMATCH: order article body')
paragraphs = []
...
checks = {
    ...
    'h1_keyword_prefix': h1_text.lower().startswith('outsourced order processing philippines'),
    ...
    'chart_present': 'class="evidence-chart"' in article and 'order-chart-title' in article,
    'graphic_present': 'class="review-graphic"' in article and 'order-graphic-title' in article,

...
MARKER = 'ph-onboarding-data-20260728'
...
body = re.search(r'<div class="article-body accounting-guide onboarding-data-guide">(.*)</div>', article, re.S)
if not body:
    raise SystemExit('AUDIT_SELECTOR_MISMATCH: onboarding article body')
paragraphs = []
...
checks = {
    ...
    'h1_keyword_prefix': h1_text.lower().startswith('philippines customer onboarding data entry'),
    ...
    'chart_present': 'class="evidence-chart"' in article and 'onboarding-chart-title' in article,
    'graphic_present': 'class="review-graphic"' in article and 'onboarding-graphic-title' in article,

...
const slugs=['crm-duplicate-contact-merge-authority-study','contact-suppression-propagation-evidence-study','meeting-action-owner-acceptance-study','recurring-report-cutoff-consistency-study','reopened-support-case-evidence-study'];
if(slugs.length!==5||new Set(slugs).size!==5)throw new Error('batch must contain exactly five unique slugs');
if(!source.includes("const published='2026-09-25'"))throw new Error('publication date missing');
...
if(process.env.CHECK_BUILT_ARTIFACTS==='1'){for(const slug of slugs){const p='.next/server/app/research/'+slug+'.html';const h=fs.readFileSync(p,'utf8');if(!h.includes('2026-09-25')||!h.includes('https://outsourcedservice.com/research/'+slug)||!h.includes('application/ld+json'))throw new Error('invalid built artifact '+slug);const text=h.replace(/<[^>]+>/g,' ').replace(/&[^;]+;/g,' ');if(text.trim().split(/\s+/).length<1200)throw new Error('under 1200 rendered words '+slug);}}
console.log('PASS: 5 unique September 25 Research articles, date, canonical, index, sitemap, structured data, and substantive length validated'+(process.env.CHECK_BUILT_ARTIFACTS==='1'?' with built artifacts':''));
